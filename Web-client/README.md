...
По умолчанию клиент ожидает, что API доступно по адресу `http://127.0.0.1:5000`. Измените переменную `VITE_API_URL` в файле `.env` при необходимости (пример приведён в `.env.example`).
